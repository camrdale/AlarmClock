...
public class Apa102 implements AutoCloseable {
    ...
    /**
     * Sets the max brightness for all LEDs in the strip.
     ...
     * @throws IOException if writing to the LEDs fails
     */
    public void write(int[] colors) throws IOException {
        ...
        for (int i = 0; i < colors.length; i++) {
            ...
            copyApaColorData(mLedBrightness, colors[di], mLedMode, mLedData, pos);
     ...
     */
    @VisibleForTesting
    static void copyApaColorData(int ledBrightness, int color, Mode ledMode, byte[] dest, int pos) {
        ...
        byte brightness = (byte) (0xE0 | (ledBrightness * a / 0xFF)); // Less brightness possible
        dest[pos] = brightness;
